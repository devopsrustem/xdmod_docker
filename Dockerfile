...
# Создание директорий
RUN mkdir -p /var/log/xdmod /var/lib/mysql /opt/xdmod-data /usr/share/xdmod/logs && \
...
COPY scripts/export-slurm-daily.sh /usr/local/bin/export-slurm-daily.sh
...
RUN chmod +x /usr/local/bin/entrypoint.sh /usr/local/bin/setup-xdmod.sh \
    ...
    /usr/local/bin/auto-import.sh /usr/local/bin/export-slurm-daily.sh
