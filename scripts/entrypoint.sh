...
# Установка UTC для всех компонентов
export TZ=UTC
...
# Настройка MySQL конфигурации для XDMoD
echo "Настройка MySQL конфигурации для XDMoD..."
cat > /etc/mysql/mariadb.conf.d/99-xdmod.cnf << 'EOF'
[mysqld]
sql_mode = ''
...
if mysql -u root -p${MYSQL_ROOT_PASSWORD} -e "SELECT 1" &>/dev/null; then
    ...
    # Настройка часового пояса MySQL на UTC
    echo "Настройка часового пояса MySQL на UTC..."
    mysql -u root -p${MYSQL_ROOT_PASSWORD} -e "SET GLOBAL time_zone = '+00:00';" 2>/dev/null || true
else
    echo "ОШИБКА: MySQL не отвечает"
    exit 1
fi

# Исправление пароля root для XDMoD setup (если нужно)
if ! mysql -u root -p${MYSQL_ROOT_PASSWORD} -e "SELECT 1" &>/dev/null; then
    echo "Исправление пароля root..."
...
# Создание символической ссылки на конфигурацию XDMoD
ln -sf /usr/share/xdmod/etc/portal_settings.ini /etc/xdmod/portal_settings.ini
...
# Дополнительное исправление пароля root после запуска
echo "Проверка и исправление пароля root..."
